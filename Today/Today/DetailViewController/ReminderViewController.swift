...
class ReminderViewController: UICollectionViewController {
    ...
    func cellRegistrationHandler(cell: UICollectionViewListCell, indexPath: IndexPath, row: Row) {
        ...
        case (.title, .editableText(let title)): cell.contentConfiguration = titleConfiguration(for: cell, with: title)
    ...
    private func section(for indexPath: IndexPath) -> Section {
        let sectionNimber = isEditing ? indexPath.section + 1 : indexPath.section
        guard let section = Section(rawValue: sectionNimber) else {
